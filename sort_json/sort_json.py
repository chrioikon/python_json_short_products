...
def write_to_items(df: DataFrame, items: DataFrame, ItemCode: int, ItemName: int, TreatmentGroup: int, heb_name: int = None, heb_brand: int = None):
    ...
    for _, row in df.iterrows():
        ...
        temp = items.loc[(items[items.columns[0]] == row[ItemCode]) & (
            (items[items.columns[1]] == "unknown") | (items[items.columns[2]] == "unknown") | (items[items.columns[1]].str.startswith("*")) | (items[items.columns[1]].isnull()) | (items[items.columns[2]].isnull()))]
        ...
        for index2, row2 in temp2.iterrows():
            try:
                ...
                items.loc[index2, "TreatmentGroup"] = re.findall(r'[B|I|S|N]\w+', row[TreatmentGroup])[
                    0][0] if re.findall(r'[B|I|S|N]\w+', row[TreatmentGroup]) else None
                items["TreatmentGroup"] = items["TreatmentGroup"].fillna('').where(pd.notnull(items["TreatmentGroup"]), None)
        ...
        if len(temp) == 0 and len(items.loc[(items[items.columns[0]] == row[ItemCode])]) == 0:
            ...
            pict = {items.columns[0]: row[ItemCode],
                    items.columns[1]: name, items.columns[2]: brand, "TreatmentGroup": re.findall(r'[B|I|S|N]\w+', row[TreatmentGroup])[0][0] if re.findall(r'[B|I|S|N]\w+', row[TreatmentGroup]) else None}
...
def evaluate_python_cond(expression: str, val: dict):
    if ('&' not in expression) and ('|' not in expression) and ('!' not in expression) and ('=' not in expression) and ('>' not in expression) and ('<' not in expression):
...
if __name__ == "__main__":
    ...
    for argum in args_list:
        if argum.split("=")[0] in ["ItemCode", "ItemPrice", "TreatmentGroup", "DoseFamily", "Type", "ItemName", "HebrewName", "HebrewBrand"]:
            a = {}
            exec(argum.split("=")[0] + "=" +
    ...
    try:
        items = write_to_items(
            df, items, values["ItemCode"]-1, values["ItemName"]-1, values["TreatmentGroup"]-1, values["HebrewName"]-1, values["HebrewBrand"]-1)
    except KeyError:
        items = write_to_items(
    ...
    json_info = pd.merge(items, df1, on="ItemCode")
